...
//form input untuk kursi reguler (kiri kanan sama)
				...
				if RegKiri[i][j].identity == 0 {
					...
					for j < N && !full {
						...
						if RegKiri[i][j].identity == 0 {
							inputdata()
							data()
						} else if RegKiri[i][4].identity != 0 {
							overload()
						}
					}
				}
				...
				if RegKanan[i][j].identity == 0 {
					...
					for j < N && !full {
						...
						if RegKanan[i][j].identity == 0 {
							inputdata()
							data()
						} else if RegKanan[i][4].identity != 0 {
							overload()
						}
					}
				}
...
//informasi data pengguna di kursi VIP
func dataVIP() {
...
//informasi data pengguna di kursi reguler kiri
func dataRegKiri() {
...
//informasi data pengguna di kursi regulerkanan
func dataRegKanan() {
...
//fungsi untuk menghandle jika suatu kursi di sebuah area sudah penuh
func overload() {
	if jenisTiket == "VIP" {
...
//cetak tiket jadi bentuk file txt
...
//searching dengan kode reservasi
func searchreserve() {
	i = 0
	j = 0
...
//searching dengan nomor identitas
func searchidentity() {
	i = 0
	j = 0
...
//menghitung jumlah semua kursi yang sudah terisi di semua area dan komposisi penonton
func okupansi() {
	vip := 0
	kanan := 0
	kiri := 0
...
//sorting semua area kursi berdasarkan umur terurut dari kecil ke besar, sorting menggunakan selection sort
func sortusia() {
	tampungVIP = VIP
...
//menghitung jumlah semua kursi kosong di semua area
func empty() {
	vip := 0
	kanan := 0
	kiri := 0
	//searching array berdasarkan indeks [N][M]
	for i := 1; i < N; i++ {
		for j := 1; j < M; j++ {
			if VIP[i][j].identity == 0 {
				vip++
...
//program utama, berisi menu pilihan
func main() {
	for nomor >= 1 && nomor < 8 {
		switch {
			//seperti if else
