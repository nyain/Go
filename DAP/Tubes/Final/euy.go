			...
			if jenisTiket == "VIP" {
				for j := 0; j < M; j++ {
					if VIP[i][j].identity == identity {
						fmt.Println("NOMOR IDENTITAS SUDAH TERDAFTAR")
					} else {
						identitysama = true
...
func dataVIP() {
	...
	fmt.Println("Nama Panggilan  : ", VIP[i][j].nama)
...
func dataRegKiri() {
	...
	fmt.Println("Nama Panggilan  : ", RegKiri[i][j].nama)
...
func dataRegKanan() {
	...
	fmt.Println("Nama Panggilan  : ", RegKanan[i][j].nama)
